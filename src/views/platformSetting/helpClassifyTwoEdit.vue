...
export default {
  ...
  methods: {
    getDetail() {
      this.$http({
        url: '/userorg/backadmin/help/ls/classify/' + this.id,
        method: 'GET',
      })
        .then(res => {
        ...
        if (valid) {
          this.$http({
            ...
            data: {
              id: this.id ? this.id : '',
              level: 2,
              fatherId: this.ruleForm.fatherId,
              name: this.ruleForm.name,
            },
          }).then(res => {
            this.$message.success(res.msg)
            this.$router.push({path: '/helpClassifyOneList',query: {classifyOneId: this.classifyOneId}})
          }).catch(e => {
            console.log(e)
          })
        } else {
